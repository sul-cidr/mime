...
async def main() -> None:
    ...
    if not os.path.isdir(f"poem_files/{video_name}"):
        logging.error(
            f"No source folder for POEM input, should be at /api/poem_files/{video_name}"
        )
        return

    # Output file from Pr_VIPE code
    poem_file = open(
        f"poem_files/{video_name}/unnormalized_embeddings.csv",
        "r",
        newline="",
        encoding="utf-8",
    )
    ...
    for pose in posesreader:
        ...
        poem_embeddings_data.append(
            [pose["video_id"], int(pose["frame"]), int(pose["pose_idx"]), poem_coords]
        )
