...
function Config() {
  ...
  this.size = {
    ...
    points: {
      // the follow values are set by Data()
...
Data.prototype.load = function () {
  get(
    getPath(config.data.dir + "/" + config.data.file),
    function (json) {
...
Data.prototype.parseManifest = function (json) {
  ...
  world.elems.pointSize.value =
    config.size.points.initial / window.devicePixelRatio;
...
Data.prototype.onTextureProgress = function (texIdx, progress) {
  this.textureProgress[texIdx] =
    progress / this.textures[texIdx].getAtlasCount();
...
Data.prototype.addCells = function (positions) {
  ...
  for (var i = 0; i < this.json.cell_sizes.length; i++) {
    // atlas index
    for (var j = 0; j < this.json.cell_sizes[i].length; j++) {
      // cell index within atlas
      drawcall.vertices++;
...
Texture.prototype.getAtlasCount = function () {
  var val =
    data.atlasCount / config.atlasesPerTex > this.idx + 1
...
Cell.prototype.setBuffer = function (attr) {
  ...
  switch (attr) {
    ...
    case "offset":
      ...
      var texSize =
        this.texIdx == -1 ? config.size.lodTexture : config.size.texture;
...
Layout.prototype.showHideIcons = function () {
  var display =
    config.isTouchDevice || config.isNarrowDevice ? "none" : "inline-block";
...
Layout.prototype.initializeUmapInputs = function () {
  ...
  this.elems.nNeighborsInput.addEventListener(
    "change",
    function () {
      this.set("umap", true);
    }.bind(this),
...
Layout.prototype.showHideUmapInputs = function () {
  if (this.selected !== "umap") {
    ...
    this.elems.nNeighborsInputContainer.style.display =
      this.getNNeighborsOptions().length > 1 ? "inline-block" : "none";
    this.elems.minDistInputContainer.style.display =
      this.getMinDistOptions().length > 1 ? "inline-block" : "none";
  }
};
...
Layout.prototype.getSelectedNNeighbors = function () {
  return this.getNNeighborsOptions()[
    parseInt(this.elems.nNeighborsInput.value)
  ];
};
...
Layout.prototype.addEventListeners = function () {
  ...
  this.elems.icons.addEventListener(
    "click",
    function (e) {
      ...
      get(
        ...
        function (pos) {
          ...
          for (var i = 0; i < data.cells.length; i++) {
            ...
            data.cells[i].tz =
              pos[i][2] || data.cells[i].getZ(pos[i][0], pos[i][1]);
...
Layout.prototype.getLayoutPath = function () {
  ...
  var selected = data.layouts[this.selected]["variants"].filter(
    function (v) {
      return (
...
Layout.prototype.showHideContext = function () {
  this.selected == "geographic" ? globe.show() : globe.hide();
};
...
Lasso.prototype.handleMouseUp = function (e) {
  ...
  if (
    coords.x == this.mousedownCoords.x &&
...
Lasso.prototype.addModalEventListeners = function () {
  ...
  this.elems.modalContainer.addEventListener(
    "click",
    function (e) {
  ...
  this.elems.viewSelected.addEventListener(
    "click",
    function (e) {
  ...
  for (var i = 0; i < this.elems.filetypeButtons.length; i++) {
    this.elems.filetypeButtons[i].addEventListener(
      "click",
      function (e) {
...
Lasso.prototype.getMesh = function () {
  ...
  var material = new THREE.RawShaderMaterial({
    ...
    fragmentShader: document.querySelector("#dashed-fragment-shader")
      .textContent,
  });
...
Lasso.prototype.getSelectedMetadata = function (callback) {
  ...
  if (data.json.metadata) {
    for (var i = 0; i < images.length; i++) {
      var metadata = {};
      get(
        config.data.dir + "/metadata/file/" + images[i] + ".json",
        function (data) {
...
Picker.prototype.onMouseUp = function (e) {
  ...
  if (e.target.className == "modal-top" || e.target.className == "modal-x") {
  ...
  if (
    Math.abs(click.x - this.mouseDown.x) > allowedDelta ||
    Math.abs(click.y - this.mouseDown.y) > allowedDelta
  )
    return;
...
Dates.prototype.load = function () {
  get(
    getPath(config.data.dir + "/metadata/dates.json"),
    function (json) {
      ...
      keys.forEach(
        function (k) {
          ...
          this.imageSelected = function (image) {
            ...
            if (
              this.state.selected[0] == this.state.min &&
              this.state.selected[1] == this.state.max
            )
              return true;
...
Modal.prototype.showPreviousCell = function () {
  if (!this.state.displayed) return;
  var cellIdx =
    this.cellIdx > 0 ? this.cellIdx - 1 : this.cellIndices.length - 1;
  this.fadeOutContent();
  setTimeout(
    function () {
...
Modal.prototype.showNextCell = function () {
  if (!this.state.displayed) return;
  var cellIdx =
    this.cellIdx < this.cellIndices.length - 1 ? this.cellIdx + 1 : 0;
  this.fadeOutContent();
  setTimeout(
    function () {
      this.showCells(this.cellIndices, cellIdx);
...
LOD.prototype.update = function () {
  if (!this.state.run || world.state.flying || world.state.transitioning)
    return;
...
LOD.prototype.fetchNextImage = function () {
  ...
  if (Number.isInteger(cellIdx)) {
      ...
      image.onload = function (cellIdx) {
        ...
        if (!this.state.cellIdxToCoords[cellIdx]) {
          this.state.cellsToActivate =
            this.state.cellsToActivate.concat(cellIdx);
        }
      }.bind(this, cellIdx);
    ...
    for (
      var x = Math.floor(-this.state.radius * 1.5);
      x <= Math.ceil(this.state.radius * 1.5);
      x++
    ) {
...
LOD.prototype.addCellsToLodTexture = function () {
  ...
  for (var i = 0; i < this.state.cellsToActivate.length; i++) {
    ...
    if (this.state.cellIdxToCoords[cellIdx] || !this.inRadius(cell.gridCoords))
      continue;
    ...
    if (coords) {
      ...
      if (!this.state.gridPosToCoords[gridKey])
...
function Filter(obj) {
  ...
  for (var i = 0; i < this.values.length; i++) {
    ...
    if (option.text.length > 22)
      option.text = option.text.substring(0, 22) + "...";
...
Filter.prototype.filterImages = function () {
  if (!this.selected) {
    ...
    var filename =
        this.selected.replace(/\//g, "-").replace(/ /g, "__") + ".json",
...
Hotspots.prototype.addEventListeners = function () {
  ...
  this.elems.createHotspot.addEventListener(
    "click",
    function () {
  ...
  this.elems.navInner.addEventListener(
    "drop",
    function (e) {
...
Hotspots.prototype.render = function () {
  ...
  for (var i = 0; i < hotspots.length; i++) {
    ...
    hotspots[i].querySelector(".hotspot-bar-inner").style.width =
      (100 * this.json[i].images.length) / data.cells.length + "%";
    // add hotspot event listeners each time they are re-rendered
    hotspots[i].querySelector(".hotspot-image").addEventListener(
      "click",
      function (idx) {
...
Hotspots.prototype.showHide = function () {
  this.elems.nav.className =
    ["umap"].indexOf(layout.selected) > -1 ? "" : "disabled";
};
...
Globe.prototype.load = function () {
  ...
  this.xScale = d3.scaleLinear().domain([-180, 180]).range([-1, 1]);

  this.yScale = d3.scaleLinear().domain([-90, 90]).range([-0.5, 0.5]);
  ...
  function addShape(parentGeometry, i) {
    ...
    i.map((j) => {
      shape.lineTo(self.xScale(j[0]), self.yScale(j[1]));
    });
  ...
  this.targets = [
    {
      ...
      text: "Order poses alphabetically by filename",
    },
    {
    ...
    },
    {
      ...
      text: "Arrange poses into metadata groups",
    },
    {
      elem: document.querySelector("#layout-geographic"),
      text: "Arrange poses using lat/lng coordinates",
    },
    {
...
AttractMode.prototype.nextEvent = function () {
  ...
  if (this.eventIndex === -3) {
    this.viewIndex =
      this.viewIndex + 1 < data.hotspots.json.length - 1
        ? this.viewIndex + 1
        : 0;
    ...
    this.timeout = setTimeout(
      function () {
        ...
        var nextLayoutIndex =
          layoutIndex + 1 < layout.options.length ? layoutIndex + 1 : 0;
...
function getPath(path) {
  ...
  base += path
    .replace("\\", "/")
    .replace(config.data.output_directory + "/", "data/");
  return base;
}
...
function dotProduct(a, b) {
  ...
  if (a.length !== b.length)
    throw Error("dotProduct requires vecs of same length");
  var s = 0;
...
function theta(a, b) {
  ...
  var radians = denom == 0 ? 0 : Math.acos(num / denom);
  return radians ? rad2deg(radians) : 0;
}
